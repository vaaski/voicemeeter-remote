...
const voicemeeter = {
  ...
  runvoicemeeter(voicemeeterType) {
    if (libvoicemeeter.VBVMR_RunVoicemeeter(voicemeeterType) === 0) return
  ...
  _getVoicemeeterType() {
    ...
    if (libvoicemeeter.VBVMR_GetVoicemeeterType(typePtr) !== 0) throw "running failed"
    ...
    case 2: // Voicemeeter Banana software
      ...
      break
    case 3:
      return VoicemeeterType.voicemeaterPotato
    default:
      // unknow software
  ...
  _getVoicemeeterVersion() {
    ...
    if (libvoicemeeter.VBVMR_GetVoicemeeterVersion(versionPtr) !== 0) throw "running failed"
  ...
  login() {
    if (!this.isInitialised) throw "await the initialisation before login"
...
parameterBusNames.forEach(name => {
  ...
  voicemeeter[`setBus${capitalizedName}`] = function(busNumber, value) {
    ...
    else voicemeeter._setParameter(InterfaceType.bus, name, busNumber, value)
  }
})
...
parameterStripNames.forEach(name => {
  ...
  voicemeeter[`setStrip${capitalizedName}`] = function(stripNumber, value) {
    ...
    else voicemeeter._setParameter(InterfaceType.strip, name, stripNumber, value)
  }
})
